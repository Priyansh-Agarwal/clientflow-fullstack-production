...
app.post('/api/voice/incoming', async (req, res) => {
  ...
  try {
    // Basic voice call handling - in production, use the VoiceAgentService
    const twimlResponse = `<?xml version="1.0" encoding="UTF-8"?>
<Response>
    <Say voice="alice">Hello! Thank you for calling ClientFlow AI. How can I help you today?</Say>
...
app.post('/api/voice/process-speech', async (req, res) => {
  ...
  try {
    ...
    let response = "I understand you need help. Let me connect you with our team.";
    
    if (confidence > 0.7) {
      if (speechResult.toLowerCase().includes('appointment')) {
        response = "I can help you schedule an appointment. What day and time would work for you?";
      } else if (speechResult.toLowerCase().includes('support')) {
...
app.get('/api/automation/workflows', async (req, res) => {
  console.log('🤖 Automation workflows requested');
  try {
    // Mock workflows - in production, integrate with n8n
    const workflows = [
      {
        id: 'lead-qualification',
        name: 'Lead Qualification Workflow',
        description: 'Automatically qualifies leads and routes them to sales team',
        status: 'active',
      ...
      },
      {
        ...
        description: 'Handles appointment scheduling and confirmations',
        status: 'active',
        triggers: ['voice_call', 'web_form'],
        last_run: new Date().toISOString()
      },
      {
        id: 'follow-up-sequence',
        name: 'Follow-up Sequence Workflow',
        description: 'Sends automated follow-up messages to customers',
        status: 'active',
        triggers: ['appointment_completed', 'purchase_made'],
        last_run: new Date().toISOString()
      }
    ];
...
app.post('/api/automation/trigger/:workflowId', async (req, res) => {
  console.log(`🚀 Triggering workflow: ${req.params.workflowId}`);
  try {
    const { workflowId } = req.params;
    const triggerData = req.body;

    // Mock workflow execution - in production, call n8n API
...
app.post('/api/whatsapp/webhook', async (req, res) => {
  ...
  try {
    // Basic webhook handling - in production, use WhatsAppService
...
app.post('/api/whatsapp/send-message', async (req, res) => {
  ...
  try {
    const { to, message, business_id } = req.body;
    ...
    // Mock WhatsApp message sending - in production, use WhatsAppService
...
app.use('*', (req, res) => {
  ...
  res.status(404).json({
    ...
    available_endpoints: [
      ...
      'POST /api/automation/trigger/:workflowId - Trigger workflow',
